...
class Order
{
	...
	//---------------------------------------
	// Добавить в ордер cryptogate_preorder_id
	//---------------------------------------
	public function AddPreorderId($order_id, $preorder_id)
	{
		$sql = 'UPDATE `' . _DB_TABLE_ORDER_ . '` SET cryptogate_preorder_id = ?i WHERE id = ?i';
		$this->db->query(
			$sql,
			$preorder_id,
			$order_id,
